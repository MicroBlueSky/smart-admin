package cn.newcapec.city.smart.core.core.node;
...
public class MenuNode implements Comparable {
    ...
    public int compareTo(Object o) {
        ...
        String levels = menuNode.getLevels();
        if (num == null) {
            num = 0;
        }
        if (levels == null) {
     ...
     * @return
     */
    private static List<MenuNode> mergeList(List<MenuNode> menuList, String rank, Map<String, List<MenuNode>> listMap) {
        //保存当次调用总共合并了多少元素
        int n;
        //保存当次调用总共合并出来的list
        ...
        for (n = menuList.size() - 1; n >=0&&menuList.get(n).getLevels().equals(rank); n--) {
